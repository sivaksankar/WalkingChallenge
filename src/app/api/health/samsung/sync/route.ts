...
export async function POST(req: Request) {
  try {
    ...
    await adminDb.runTransaction(async (tx: any) => {
      ...
      tx.set(stepsRef, {
        steps: Math.max(currentSteps, steps), // Use the higher value
        date,
    ...
    // Log sync activity
    await adminDb.collection('users').doc(userId).collection('health_syncs').add({
      source: 'samsung_health',
      date,
      steps,
      syncedAt: new Date()
    ...
    return NextResponse.json({ success: true, message: 'Samsung Health data synced successfully' });
  } catch (error: any) {
    console.error('Error in /api/health/samsung/sync:', error);
