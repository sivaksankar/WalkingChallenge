...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      // error in the location so the SPA can react.
      const html = `<!doctype html><html><head><meta charset="utf-8"><title>Auth Error</title></head><body><h1>Sign-in error</h1><p>${encodeURIComponent(error)}</p><script>localStorage.setItem('auth_error', '${encodeURIComponent(error)}');window.location='/'</script></body></html>`
    ...
    if (!code) {
      ...
      const html = `<!doctype html><html><head><meta charset="utf-8"><title>Auth Error</title></head><body><h1>Sign-in error</h1><p>Missing code</p><script>localStorage.setItem('auth_error', 'missing_code');window.location='/'</script></body></html>`
      return new Response(html, { headers: { 'Content-Type': 'text/html' } })
    }
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Signing in...</title></head><body><p>Signing you in — returning to the app.</p><script>localStorage.setItem('auth_code', '${encodeURIComponent(code)}');window.location='/'</script></body></html>`
